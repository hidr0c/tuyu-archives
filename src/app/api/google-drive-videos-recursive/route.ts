  ...
  try {
    ...
    for (const folder of allFolders) {
      try {
        ...
        console.error(`Error getting files from folder ${folder.path}:`, error.message);
      }
    }
    ...
    const subtitleFiles = Array.from(allFiles.values()).filter(file => 
      file.name.toLowerCase().endsWith('.vtt') || 
      file.name.toLowerCase().endsWith('.srt'));
    
    console.log(`Found ${videoFiles.length} videos and ${subtitleFiles.length} subtitles`);
